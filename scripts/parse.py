...
from typing import Optional, NewType, Iterable, Set, Dict
import csv
import io
import os
import struct
...
TAConstantEntry = NewType('TAConstantEntry', Set[str])
TAConstants = NewType('TAConstants', Dict[int, TAConstantEntry])
        ...
        for row in reader:
            try:
                ...
                if val not in constants:
                    constants[val] = set()
                constants[val].add(row[1])
            except ValueError:
                pass
            
    return constants

knownTAConstants = read_taconstants('parse_data/weaponids.csv')
